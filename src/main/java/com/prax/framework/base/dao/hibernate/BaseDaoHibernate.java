...
public class BaseDaoHibernate<T> extends HibernateDaoSupport implements HibernateDao<T> {
	...
	public Page<T> findPage(final Page<T> page, final Criterion... criterions) {
		...
		for (T o : result) {
			fetchProperties(o, page.getFetchProps());
		}
		page.setResult(result);
		return page;
	}
	...
	public Page<T> findPage(final Page<T> page, final List<PropertyFilter> filters) {
		...
		Criterion[] criterions = buildPropertyFilterCriterions(filters, aliasMap);

		Criteria c = createCriteria(criterions);

		// order properties alias
		if (page.isOrderBySetted()) {
			String[] orderByArray = StringUtils.split(page.getOrderBy(), ',');
			String[] orderArray = StringUtils.split(page.getOrder(), ',');
			for (int i = 0; i < orderByArray.length; i++) {
				String aliasedProperty = getAliasedProperty(orderByArray[i], aliasMap);
				if (Page.ASC.equals(orderArray[i])) {
					c.addOrder(Order.asc(aliasedProperty));
				}
				else {
					c.addOrder(Order.desc(aliasedProperty));
				}
			}
		}
		...
		for (Entry<String, String> entry : aliasMap.entrySet()) {
			String associationPath = (String) entry.getKey();
			c.createAlias(associationPath, entry.getValue(), CriteriaSpecification.LEFT_JOIN);
		}
	 ...
	 */
	protected Criterion buildCriterion(String propertyName, Object propertyValue, final MatchType matchType,
			HashMap<String, String> aliasMap) {

		propertyName = getAliasedProperty(propertyName, aliasMap);
	...
	private String getAliasedProperty(String propertyName, HashMap<String, String> aliasMap) {
		if (propertyName.contains(".")) {
			String[] aliasesArray = propertyName.split("\\.");
			String aliasPath;
			String aliasedProperty;
			int i = 0;
			for (; i < aliasesArray.length - 1; i++) {
				if (i == 0) {
					if (aliasMap.get(aliasesArray[i]) == null) {
						aliasMap.put(aliasesArray[i], (aliasesArray[i] + i));
					}
				}
				else {
					aliasPath = (aliasesArray[i - 1] + (i - 1)) + "." + aliasesArray[i];
					if (aliasMap.get(aliasPath) == null) {
						aliasMap.put(aliasPath, (aliasesArray[i] + i));
					}
				}
			}
			aliasedProperty = (aliasesArray[i - 1] + (i - 1)) + "." + aliasesArray[i];
