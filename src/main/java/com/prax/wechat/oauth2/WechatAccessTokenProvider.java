 ...
 * @author Dave Syer
 */
public class WechatAccessTokenProvider extends OAuth2AccessTokenSupport implements AccessTokenProvider {
	...
	private ClientAuthenticationHandler authenticationHandler = new WechatClientAuthenticateHandler();
	...
	protected OAuth2AccessToken retrieveToken(MultiValueMap<String, String> form, HttpHeaders headers,
		...
		try {
			...
			return getRestTemplate().execute(getAccessTokenUri(resource, form), getHttpMethod(),null, getResponseExtractor() , new HashMap<String, String>());
		}
	...
	protected ResponseExtractor<OAuth2AccessToken> getResponseExtractor() {
		ResponseExtractor<OAuth2AccessToken> extractor = new ResponseExtractor<OAuth2AccessToken>() {
			public OAuth2AccessToken extractData(ClientHttpResponse response) throws IOException {
				...
				return (OAuth2AccessToken)converter.read(OAuth2AccessToken.class, response);
			}
		};
	...
	protected String getAccessTokenUri(OAuth2ProtectedResourceDetails resource, MultiValueMap<String, String> form) {
		...
		Map<String, String> requestParams = form.toSingleValueMap();
		char appendChar = accessTokenUri.indexOf('?') < 0 ? '?' : '&';
